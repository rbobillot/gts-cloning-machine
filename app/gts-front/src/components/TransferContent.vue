    ...
    if (typeof move === 'string') {
      axios
        ...
        .then((moveResp) => {
          p.moves[index] = {
            index: index,
            name: move, // name: moveResp.data.names.find((n: any) => n.language.name === 'fr').name,
...
<template>
  ...
  <div v-if="tStore.shouldDisplayTransferContent" class="transfer-content">
    <!-- Pokemon basic info props -->
    <div class="pkmn-sprite-star-object-nature">
      <div class="pkmn-sprite" width="96" height="96">
        <img v-tooltip.top="tStore.selectedPkmn?.species" :src="getPokemonSpriteUrl()" />
      </div>
      ...
      <div class="pkmn-held-item">
        <img v-if="tStore.isHoldingItem"
        v-tooltip.top="tStore.selectedPkmn?.holding"
        :src="`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/${tStore.selectedPkmnHeldItem?.replace(' ', '-')}.png`" />
      </div>
      <div class="pkmn-name">
    ...
    <div class="pkm-stats index-level-nature">
    ...
    <div class="pkm-stats ability-hiddenpower-happiness">
      ...
      <div class="pkm-stat pkmn-hiddenpower"><div class="pkm-stat-title">Hidden Power</div><div class="pkm-stat-value">{{tStore.selectedPkmnHiddenPower}}</div></div>
    ...
    <div class="pkm-stats ot-tid-sid-remove">
    ...
    <div class="pkmn-moves">
      <span v-for="index in 4" :key="index" :class="`lv-buttonset pkmn-move-${index}`">
        <LvButton v-if="tStore.selectedPkmn?.moves"
        ...
        <LvButton v-else
          :disabled="true"
          :style="getDamageClassColor('')"
          :icon="getDamageClassIcon('')" />
      </span>
    </div>
