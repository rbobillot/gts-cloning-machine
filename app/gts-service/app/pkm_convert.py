...
def get_nickname(namebytes, origin_game) -> str:
    """
    --- GEN 4
...
def update_field(pkm, field, gen_convert) -> bytes:
    """
    Can either modify the nickname or the OT name
    gen_convert is a string like "4:5" or "5:4"
    """
    p = array('B')
    p.frombytes(pkm)
    ...
    def modify_name_in_range(name, start, end):
        ...
        for i in range(start, end, 2):
            index = int((i - start) / 2) # new name's index
            length = len(name)           # new name's length
            if index < length:
                if gen_convert == "5:4":
                ...
                elif gen_convert == "4:5":
                    p[i] = ord(name[index]) & 0xff
                    p[i+1] = ord(name[index]) >> 8
    ...
    if 'name' == field:
        modify_name_in_range(get_nickname(p[0x48:0x5e], origin_game), 0x48, 0x5e)
    elif 'ot' == field:
        modify_name_in_range(get_nickname(p[0x68:0x78], origin_game), 0x68, 0x78)

    update_checksum()

    return p.tobytes()

def get_pkm_origin_game(pkm) -> str:
    return gorigin.get(pkm[0x5f])

def get_pkm_gen(pkm) -> int:
    p = array('B')
    p.frombytes(pkm)

    is_gen_5 = any(gen5 in get_pkm_origin_game(pkm) for gen5 in ['White','Black']) # includes Black 2 and White 2

    return 5 if is_gen_5 else 4
    """
    # Get the game origin of the pokemon (from the Nat.Dex ID)
    nat_dex_id = (p[0x09] << 8) + p[0x08]
    gen = 0

    if   nat_dex_id <= 151: gen = 1
    elif nat_dex_id <= 251: gen = 2
    elif nat_dex_id <= 386: gen = 3
    elif nat_dex_id <= 493: gen = 4
    elif nat_dex_id <= 649: gen = 5
    elif nat_dex_id <= 721: gen = 6
    elif nat_dex_id <= 809: gen = 7
    elif nat_dex_id <= 898: gen = 8

    # Maybe, combine the 2 methods to get the gen (orgin_game + nat_dex_id)
    #   To know if the pokemon can be converted from gen5 to gen4
    #   (when the origin game is BW or B2W2, and the nat_dex_id is <= 493)
    return (gen, get_pkm_origin_game(pkm))
    """

def pkm_convert_to_gen(pkm, target_gen) -> bytes:
    """
    Convert a Pokemon from one generation to another
    Actually, it only converts gen4 to gen5, by updating Pokemon nickname and OT name
    Maybe, gen5 to gen4 should be handled
      (when Pokemon coming from gen5 cartridge, is actually a gen4 or older Pokemon)
    """
    p = array('B')
    p.frombytes(pkm)

    pkm_gen = get_pkm_gen(pkm)

    if pkm_gen == target_gen:
        return pkm
    elif pkm_gen == 4 and target_gen == 5:
        if len(p) == 236:
            p = p[0:220]
        p = update_field(p, 'name', "4:5")
        p = update_field(p, 'ot', "4:5")
        """
        # TODO [Gen5 handling]: avoid gen5 exclusive Pokemon to be converted to gen4
        # and avoid conversion of gen4 Pokemon with gen5 attacks/abilities/types...
        elif pkm_gen == 5 and target_gen == 4:
            if len(p) == 220:
                padding = array('B', [0] * 16)
                p = p + padding
            p = update_field(p, 'name', "5:4")
            p = update_field(p, 'ot', "5:4")
        """
    else: # it should not happen (this is handled by gts-front)
        raise Exception(f"Unsupported conversion: {pkm_gen} -> {target_gen}")
...
def pkm_to_json(pkm):
    ...
    raw_pkm_data_b64 = b64encode(p).decode("utf-8")
    ...
    # Block C
    origin_game = get_pkm_origin_game(p)
